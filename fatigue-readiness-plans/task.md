...
- [ ] **3.3** Handle fallback for sessions without second-by-second data:
  ...
  - **Steady-state special case**: No W' depletion, simplified cost = power × duration
- [ ] **3.4** Extract second-by-second data from guided sessions:
  - Modify `useSessionTimer.ts` to capture at 5-second intervals
  - Update `powerHistory` format in `SessionResult`
- [ ] **3.5** Write comprehensive unit tests in `physiologicalCostEngine.test.ts`

---

## Phase 4: Dual-Compartment Chronic State
> Metabolic Freshness (MET) and Structural Health (MSK)

- [ ] **4.1** Create `utils/chronicFatigueModel.ts`:
  - `updateMetabolicFreshness()` - MET dynamics with τ_meta ≈ 2 days
  - `updateStructuralHealth()` - MSK dynamics with τ_struct ≈ 15 days
  - `calculateReadinessScore()` - Composite score from both compartments
...
- [ ] **5.1** Modify `utils/questionnaireConfig.ts`:
...
- [ ] **5.2** Implement Recovery Efficiency Modulation:
  - Apply φ_recovery to τ_meta in MET decay equation
...
- [ ] **5.3** Implement Bayesian State Corrections:
  - Structural Correction (Soreness): Force-inject MSK if model mismatch
...
- [ ] **5.5** Update tests in `questionnaireAlgorithm.test.ts`
...
- [ ] **6.2** Implement penalty load system:
  - If RPE >> predicted → apply penalty load to MET for 24 hours
- [ ] **6.3** Implement CP downgrade trigger:
  - If mismatch persists for >3 sessions → trigger CP ↓2%
- [ ] **6.4** Integrate with session completion flow in `hooks/useSessionTimer.ts`
...
- [ ] **7.1** Refactor `hooks/useMetrics.ts`:
...
- [ ] **7.2** Update `MetricsResult` interface:
...
- [ ] **8.1** Update `components/Chart.tsx`:
  - Replace ATL/CTL lines with MET/MSK visualizations
  - Add eCP trend line
  - Update tooltip content for new metrics
- [ ] **8.2** Update `generateMetrics()` function:
...
- [ ] **9.1** Update `components/DashboardTab.tsx`:
...
- [ ] **10.1** Update `utils/simulationEngine.ts`:
  - Use new chronic fatigue model in simulations
  - Output MET and MSK percentiles
- [ ] **10.2** Update `utils/autoAdaptiveModifiers.ts`:
  - Base adjustments on dual-compartment state
  - Different adjustments for metabolic vs structural issues
- [ ] **10.3** Update `utils/autoAdaptiveTypes.ts`:
  - Update `WeekPercentiles` for new model metrics
...
- [ ] **12.2** Create `CHRONIC_FATIGUE_MODEL.md`:
  - In-app developer documentation
  - Mathematical formulas reference
  - Calibration guidance
- [ ] **12.3** Update `TEMPLATE_DOCUMENTATION.md`:
  - Replace Banister model section with dual-compartment model
  - Update fatigue/readiness score formulas and tables
  - Add Critical Power estimation section
  - Update research references
- [ ] **12.4** Update code comments throughout affected files
...
- [ ] **14.1** Create migration utility:
  ...
  - Graceful handling: run backfill on first app load post-update
- [ ] **14.2** Remove legacy model code (immediate replacement, no feature flag)
