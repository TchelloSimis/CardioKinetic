...
import { FatigueModifier, ProgramTemplate, WeekDefinition } from '../programTemplate';
...
const ProgramPreview: React.FC<ProgramPreviewProps> = ({
    ...
    // Calculate week options based on weekConfigType
    const weekOptions = (() => {
        if (editorState.weekConfigType === 'custom' && editorState.customDurations.length > 0) {
            return editorState.customDurations;
        } else if (editorState.weekConfigType === 'variable') {
            return Array.from({ length: Math.floor((editorState.rangeMax - editorState.rangeMin) / editorState.rangeStep) + 1 }, (_, i) => editorState.rangeMin + i * editorState.rangeStep);
        }
        return [editorState.fixedWeeks];
    })();
    ...
    // Generate week definitions based on structure type
    // This is used by both the chart and the Week Definitions table
    const previewWeekDefs: WeekDefinition[] = (() => {
        const durationMins = editorState.defaultDurationMinutes || 15;

        if (editorState.structureType === 'block-based') {
            // For block-based programs, expand blocks to weeks
            if (!editorState.programBlocks || editorState.programBlocks.length === 0) {
                // No blocks defined - return weeks from fixedFirstWeek/fixedLastWeek if available
                const fallbackWeeks: WeekDefinition[] = [];
                if (editorState.fixedFirstWeek) {
                    fallbackWeeks.push({ ...editorState.fixedFirstWeek, position: 1 });
                }
                if (editorState.fixedLastWeek && selectedWeekCount > 1) {
                    fallbackWeeks.push({ ...editorState.fixedLastWeek, position: selectedWeekCount });
                }
                return fallbackWeeks;
            }

            // Create a temporary template object for the expansion function
            const tempTemplate: ProgramTemplate = {
                templateVersion: '1.0',
                id: 'preview',
                name: editorState.name,
                description: editorState.description,
                weekConfig: { type: 'fixed', fixed: selectedWeekCount },
            ...
            // Use the block expansion utility to generate weeks
            return expandBlocksToWeeks(tempTemplate, selectedWeekCount, basePower);
        } else {
            // For week-based programs, use the existing weeks array
    ...
    // Generate chart data from week definitions
    const chartData = (() => {
        if (previewWeekDefs.length === 0) return [];

        const data: { week: number; power: number; work: number; phase: string }[] = [];
        const durationMins = editorState.defaultDurationMinutes || 15;

        for (const weekDef of previewWeekDefs) {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-3">
                    {/* Show week selector for variable or custom week configs with multiple options */}
                    {(editorState.weekConfigType === 'variable' ||
                        (editorState.weekConfigType === 'custom' && weekOptions.length > 1)) && (
                            <div className="flex items-center gap-2">
            ...
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 p-6">
                <h4 className="text-sm font-bold uppercase tracking-widest text-neutral-400 mb-4">
                    {editorState.structureType === 'block-based'
                        ? `Expanded Weeks (${previewWeekDefs.length} weeks)`
                        : `Week Definitions (${editorState.weeks.length} keyframes)`
                    }
                </h4>
                {previewWeekDefs.length === 0 ? (
                    <div className="text-center py-8 text-neutral-400">
                        <p className="text-sm">
                            {editorState.structureType === 'block-based'
                                ? 'No program blocks defined. Add blocks in the Block Structure step to see the expansion.'
                                : 'No week definitions added yet.'
                            }
                        </p>
                    </div>
                ) : (
